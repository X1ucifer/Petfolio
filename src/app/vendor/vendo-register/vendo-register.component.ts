...
@Component({
  selector: 'app-vendo-register',
  templateUrl: './vendo-register.component.html',
  styleUrls: ['./vendo-register.component.css']
})
export class VendoRegisterComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.otp == this.res_otp) {
      if(this.otp_input == "email"){
        this.showSuccess("Email ID Verified Successfully")
        this.displayBasic = false;
        this.email_verification = true;
        this.ngOtpInput.setValue(undefined);
      }
      else{
        this.showSuccess("Mobile Number Verified Successfully")
        this.displayBasic = false;
        this.email_verification = true;
        this.ngOtpInput.setValue(undefined);
        this.storage.set('user', this.user_data.Data.user_details);
        this.router.navigateByUrl('/vendor_business_info')
      }
    }
    else {
